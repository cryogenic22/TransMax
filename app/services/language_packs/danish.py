...
class DanishPack(BaseLanguagePack):
    @property
    def code(self) -> str:
        return "da"
    ...
    def check_negation(
        ...
        negation_markers = ["ikke", "ingen", "aldrig", "nej"]
        ...
        if source_negation and not target_has_negation:
            ...
            violations.append(
                {
                    "type": v_type.value,
                    "message": f"Source contains negation '{source_negation.group(0)}' but Target lacks Danish negation markers.",
    ...
    def check_numbers(self, source: str, target: str) -> List[Dict[str, object]]:
        # Word-bounded numeric check (TMX-3410). Danish uses comma decimal.
        violations = []
    ...
    def prompt_instruction(self) -> str:
        return "ENSURE HIGH RIGOR: Use formal Danish suitable for pharmaceutical/regulatory (EMA) contexts."
