...
from .dutch import DutchPack
from .swedish import SwedishPack
from .danish import DanishPack
from .finnish import FinnishPack
from .polish import PolishPack
from .greek import GreekPack
from .czech import CzechPack
from .hungarian import HungarianPack
...
class GenericLanguagePack(BaseLanguagePack):
    ...
    def check_negation(
        ...
        target_has_neg = any(
            f" {w} " in f" {target_lower} "
            for w in negators.get("fr", []) + negators["en"]
        )
    ...
    def check_placeholders(self, source: str, target: str) -> List[Dict[str, Any]]:
        ...
        if diff_added:
            ...
            v_type = (
                ViolationType.PLACEHOLDER_CORRUPTION
            )  # Same type? Or HALLUCINATION?
...
class LanguagePackFactory:
    _packs = {
        ...
        # Tier 1: EMA official-language deep packs (TMX-LANG-EU1/EU2).
        # Negation + decimal-comma numeric gates, replacing the generic fallback
        # for languages required across EMA QRD package leaflets.
        "nl": DutchPack(),
        "sv": SwedishPack(),
        "da": DanishPack(),
        "fi": FinnishPack(),
        ...
        "hu": HungarianPack(),
        # Tier 1 Variants
        "fr-ca": GenericLanguagePack(
            "Use Canadian French (fr-CA) terminology and conventions."
        ),
        # Tier 1 (Generic fallback)
        "en": GenericLanguagePack(),
        "ru": GenericLanguagePack(
            "ENSURE HIGH RIGOR: Use formal Russian suitable for medical/pharmaceutical regulatory contexts."
        ),
        "it": GenericLanguagePack(
            "ENSURE HIGH RIGOR: Use formal Italian suitable for pharmaceutical documents per AIFA guidelines."
        ),
        # Tier 2: Generic Support (Universal Gates)
        "hi": GenericLanguagePack(
            "ENSURE HIGH RIGOR: Use formal, grammatically precise Hindi suitable for professional/medical contexts. Avoid colloquialisms."
        ),
        "ta": GenericLanguagePack(
            "ENSURE HIGH RIGOR: Use formal, grammatically precise Tamil suitable for professional/medical contexts. Avoid colloquialisms."
        ),
        ...
        # nl / pl / el promoted to Tier-1 deep packs above (TMX-LANG-EU1/EU2).
    }
