...
class GreekPack(BaseLanguagePack):
    @property
    def code(self) -> str:
        return "el"
    ...
    def check_negation(
        ...
        negation_markers = ["δεν", "μην", "όχι", "ποτέ", "ούτε"]
        ...
        if source_negation and not target_has_negation:
            ...
            violations.append(
                {
                    "type": v_type.value,
                    "message": f"Source contains negation '{source_negation.group(0)}' but Target lacks Greek negation markers.",
    ...
    def check_numbers(self, source: str, target: str) -> List[Dict[str, object]]:
        # Word-bounded numeric check (TMX-3410). Greek uses comma decimal.
        violations = []
    ...
    def prompt_instruction(self) -> str:
        return "ENSURE HIGH RIGOR: Use formal Greek suitable for pharmaceutical/regulatory (EMA) contexts."
