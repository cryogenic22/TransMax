...
| TMX-3050 | Sanitise Tiptap RichTextEditor egress via DOMPurify (audit F-H03 — stored XSS) | F-H03 (2026-05-09 audit) | Reviewer Frontend | 2 — **[Done]** (local; awaiting push) — see `.context/loops/TMX-3050.md`. New `frontend/lib/sanitizeHtml.ts` with explicit allowlist + URI-scheme guard + two-pass sanitisation; `RichTextEditor` `onUpdate` now wraps `editor.getHTML()` in `sanitizeRichTextHtml()`. 15 new vitest cases (12 negative XSS-vector + 5 positive legitimate-markup). Frontend 104/104 (was 89/89), typecheck 0, lint 0, build green, ratchet 16/17 (one pre-existing TODO regression unrelated). Spawned TMX-3050a (audit-event hook), TMX-3050b (backend HTML sanitisation), TMX-3050c (initial-load sanitisation). |
